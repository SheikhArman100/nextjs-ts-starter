  ...
  implements KTDataTableInterface
{
  ...
  private _initTableHeader(): void {
    ...
    headers.forEach((header) => {
      ...
      const sortAttribute =
        header.getAttribute('data-datatable-column-sort') ||
        header.getAttribute('data-datatable-column');
    ...
    const th =
      typeof sortField === 'number'
        ...
        : (this._theadElement.querySelector(
            `th[data-datatable-column="${String(sortField)}"], th[data-datatable-column-sort="${String(sortField)}"]`,
          ) as HTMLElement);
  ...
  public static createInstances(): void {
    ...
    elements.forEach((element) => {
      if (
        element.hasAttribute('data-datatable') &&
