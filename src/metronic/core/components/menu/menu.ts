...
export class KTMenu extends KTComponent implements KTMenuInterface {
  ...
  protected _isItemSubElement(itemElement: HTMLElement): boolean {
    return (
      itemElement.classList.contains('menu-dropdown') ||
      itemElement.classList.contains('menu-accordion')
    );
  }
  ...
  protected _getItemSubElement(itemElement: HTMLElement): HTMLElement {
    if (
      itemElement.classList.contains('menu-dropdown') === true ||
      itemElement.classList.contains('menu-accordion') === true
    ) {
      return itemElement;
  ...
  protected _getItemElement(element: HTMLElement): HTMLElement {
    if (
      ...
      element.hasAttribute('data-menu-item-toggle')
    ) {
      return element;
    }
  ...
  public static getInstance(element: HTMLElement): KTMenu {
    ...
    if (
      ...
      const subElement = (element.closest('.menu-dropdown') ||
        element.closest('.menu-accordion')) as HTMLElement;
  ...
  public static getFocusLink(
    ...
    if (direction === 'next') {
      ...
      if (
        nextElement &&
        (nextElement.matches('.menu-accordion' + (!preFocus ? '.show' : '')) ||
          nextElement.matches('.menu-dropdown' + (!preFocus ? '.show' : '')))
      ) {
      ...
      if (prevElement) {
        ...
        if (
          ...
          (parentElement.matches(
            '.menu-accordion' + (!preFocus ? '.show' : ''),
          ) ||
            parentElement.matches(
              '.menu-dropdown' + (!preFocus ? '.show' : ''),
            ))
        ) {
          const prevElement2 =
